...
- 只讓首頁「Across research and industry」標題隨捲動由 88% 放大至 100%（依使用者回饋由原先 96% 起點加強），以左上為基準，使用 `view()` 時間軸與 `entry 0% cover 35%` 範圍；完成後保持原尺寸，反向捲動依相同進度縮回。標題維持完全不透明，不改變排版字級或正文。

桌面與手機均啟用標題縮放；不支援所需時間軸／範圍的瀏覽器顯示原尺寸。所有位移、縮放與過渡只在 `prefers-reduced-motion: no-preference` 啟用；減少動態時保留即時顏色、底線和焦點。Hover 樣式限 `(hover: hover) and (pointer: fine)`，觸控仍有按壓回饋。姓名、頭像、正文和其他標題保持穩定，不加入換頁、進場、視差或連續循環動畫。
